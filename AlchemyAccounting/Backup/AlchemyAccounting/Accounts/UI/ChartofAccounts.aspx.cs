        ...
        protected void txtExpen_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    if (txtExpen.Text != "")
                    {
                        Global.txtAdd(@"Select ACCOUNTCD from GL_ACCHART where ACCOUNTCD like '4%'  AND ACCOUNTNM = '" + txtExpen.Text + "'", txtCode);
                    }
                    else
                        txtCode.Text = "";
                    lblLvlID.Visible = true;
                    Global.lblAdd(@"Select LEVELCD from GL_ACCHART where ACCOUNTNM='" + txtExpen.Text + "'", lblLvlID);
                    lblBotCode.Text = "";
                    lblBotCode.Text = (Convert.ToDecimal(lblLvlID.Text) + 1).ToString();
                    gvDetails.Visible = false;
                    btnSubmit.Focus();
