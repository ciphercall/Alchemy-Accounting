...
namespace AlchemyAccounting.Accounts.Report.Report
{
    public partial class rptBalanceSheet : System.Web.UI.Page
    {
        ...
        public void showgrid_Liability()
        {
            ...
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = (" SELECT A.ACCD1, GL_ACCHART.ACCOUNTNM AS ACCDNM1, A.ACCD2, GL_ACCHART_1.ACCOUNTNM AS ACCDNM2 , GL_ACCHART_2.ACCOUNTNM AS ACCDNM3, A.ACCD3, A.AMT " +
                               " FROM GL_ACCHART INNER JOIN (SELECT SUBSTRING(DEBITCD, 1, 1) + '00000000000' AS ACCD1, SUBSTRING(DEBITCD, 1, 3) + '000000000' AS ACCD2, " +
                               " SUBSTRING(DEBITCD, 1, 5) + '0000000' AS ACCD3, SUM(DEBITAMT) - SUM(CREDITAMT) AS AMT FROM GL_MASTER " +
                               " WHERE (SUBSTRING(DEBITCD, 1, 1) = 2) AND (TRANSDT <= '" + st_DT + "') " +
                               " GROUP BY SUBSTRING(DEBITCD, 1, 1) + '00000000000', SUBSTRING(DEBITCD, 1, 3) + '000000000', SUBSTRING(DEBITCD, 1, 5) + '0000000' " +
                               " UNION " +
                               " SELECT '2' + '00000000000' AS ACCD1, '201' + '000000000' AS ACCD2, '20102' + '0000000' AS ACCD3, SUM(CREDITAMT) - SUM(DEBITAMT) AS AMT " +
                               " FROM GL_MASTER AS GL_MASTER_1 WHERE (SUBSTRING(DEBITCD, 1, 1) IN (3, 4)) AND (TRANSDT <= '" + st_DT + "')) AS A " +
