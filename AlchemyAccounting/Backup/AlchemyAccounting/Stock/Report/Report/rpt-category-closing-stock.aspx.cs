...
{
    public partial class rpt_category_closing_stock : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string catNM = Session["catNM"].ToString();
                string cateID = Session["cateID"].ToString();
        ...
        public void showGrid()
        {
            ...
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT     B.CATID, B.ITEMID, B.CLQTY, B.AVGRATE, (CASE WHEN B.CLQTY < 0 THEN 0.00 ELSE B.CLQTY * B.AVGRATE END) AS STOCKVALUE, STK_ITEMMST.CATNM, " +
                      " STK_ITEM.ITEMNM, STK_ITEM.PQTY, (CASE WHEN STK_ITEM.PQTY = 0 THEN 0 ELSE FLOOR(B.CLQTY / PQTY) END) AS CARTONQTY, B.CLQTY - (CASE WHEN STK_ITEM.PQTY = 0 THEN 0 ELSE (FLOOR(B.CLQTY / PQTY) * PQTY) END) AS PIECES " +
                      " FROM (SELECT CATID, ITEMID, (SUM(INQTY) + SUM(BQTY)) - (SUM(OUTQTY) + SUM(SQTY)) AS CLQTY, SUM(BQTY) AS BQTY, SUM(BAMT) AS BAMT, SUM(SQTY) AS SQTY, SUM(SAMT) AS SAMT, (CASE WHEN SUM(BAMTR) = 0.00 THEN 0.00 ELSE CONVERT(decimal(18, 2), (SUM(BAMTR)) / SUM(BQTYR)) END) AS AVGRATE " +
                              " FROM (SELECT CATID, ITEMID, SUM(QTY) AS BQTY, SUM(AMOUNT) AS BAMT, 0 AS SQTY, 0 AS SAMT, 0 AS INQTY, 0 AS OUTQTY, 0 AS BQTYR,  0 AS BAMTR FROM STK_TRANS " +
                                               " WHERE      (TRANSDT <= '" + aOn + "') AND (TRANSTP = 'BUY') GROUP BY CATID, ITEMID " +
                                               " UNION " +
                                               " SELECT     CATID, ITEMID, 0 AS BQTY, 0 AS BAMT, SUM(QTY) AS SQTY, SUM(AMOUNT) AS SAMT, 0 AS INQTY, 0 AS OUTQTY, 0 AS BQTYR, 0 AS BAMTR " +
                                               " FROM         STK_TRANS AS STK_TRANS_1 WHERE     (TRANSDT <= '" + aOn + "') AND (TRANSTP = 'SALE') " +
                                               ...
                                               " GROUP BY CATID, ITEMID " +
                                                " UNION " +
                                               " SELECT     CATID, ITEMID, 0 AS BQTY, 0 AS BAMT, 0 AS SQTY, 0 AS SAMT, 0 AS INQTY, 0 AS OUTQTY, SUM(QTY) AS BQTYR, SUM(AMOUNT)  " +
                                                                     " AS BAMTR " +
                                               " FROM         STK_TRANS " +
...
" WHERE     (B.CLQTY <> 0) AND B.CATID='" + cateID + "' ORDER BY ITEMNM", conn);
