...
namespace AlchemyAccounting.CNF.PartyCommission
{
    public partial class commission : System.Web.UI.Page
    {
        ...
        public void GridShow()
        {
            ...
            cmdd = new SqlCommand("SELECT CNF_COMMISSION.COMMSL, CNF_COMMISSION.EXCTP, CNF_COMMISSION.VALUEFR, CNF_COMMISSION.VALUETO, CNF_COMMISSION.VALUETP, CNF_COMMISSION.COMMAMT, CNF_COMMISSION.JOBTP FROM CNF_COMMISSION INNER JOIN GL_ACCHART ON CNF_COMMISSION.PARTYID = GL_ACCHART.ACCOUNTCD WHERE GL_ACCHART.ACCOUNTCD='" + txtPartyID.Text + "'", conn);
        ...
        protected void gvDetails_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            DropDownList ddlType = (DropDownList)gvDetails.FooterRow.FindControl("ddlJobType");
            ...
            if (e.CommandName.Equals("SaveCon"))
            {
                ...
                else
                {
                    ...
                    pcm.JobType = ddlType.SelectedValue;
        ...
        protected void gvDetails_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            SqlCommand cmdselectdata = new SqlCommand("SELECT * FROM CNF_COMMISSION WHERE PARTYID =@PARTYID AND COMMSL =@COMMSL", conn);
            cmdselectdata.Parameters.Clear();
            cmdselectdata.Parameters.Add("@PARTYID", SqlDbType.NVarChar).Value = pcm.PARTYID;
            cmdselectdata.Parameters.Add("@COMMSL", SqlDbType.BigInt).Value = pcm.COMMSL;

            SqlDataReader dr = cmdselectdata.ExecuteReader();
            while (dr.Read())
            {
                PARTYID = dr["PARTYID"].ToString();
                COMMSL = dr["COMMSL"].ToString();
                EXCTP = dr["EXCTP"].ToString();
                VALUEFR = dr["VALUEFR"].ToString();
                VALUETO = dr["VALUETO"].ToString();
                VALUETP = dr["VALUETP"].ToString();
                COMMAMT = dr["COMMAMT"].ToString();
            ...
            string alldata = PARTYID + ", " + COMMSL + ", " + EXCTP + ", " + VALUEFR + ", " + VALUETO + ", " + VALUETP
                + ", " + COMMAMT + ", " + USERPC + ", " + INTIME + ", " + UPDATETIME + ", " + IPADDRESS;

            pcm.InTM = DateTime.Now;


            SqlCommand cmdinsert = new SqlCommand("insert into ASL_DLT values('CNF_COMMISSION',@DESCRP,@USERPC,@USERID,@INTIME,@IPADD)", conn);
            ...
            cmdinsert.Parameters.AddWithValue("@INTIME", pcm.InTM);
        ...
        protected void gvDetails_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            DropDownList ddlType = (DropDownList)gvDetails.Rows[e.RowIndex].FindControl("ddlJobTypeEdit");
