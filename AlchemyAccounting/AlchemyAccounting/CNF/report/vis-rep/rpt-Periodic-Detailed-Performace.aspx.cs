...
WHERE (dbo.CNF_JOB.JOBCDT BETWEEN @FDT AND @TDT) AND (dbo.CNF_JOBBILL.JOBTP = @JOBTP) AND (dbo.CNF_JOBBILL.COMPID = @COMPID)
...
AS C 
GROUP BY C.ACCOUNTNM,C.JOBNO, C.BILLDT, C.BILLNO, C.BILLAMT, C.JOBYY, C.JOBTP,  C.FRWNO,C.EXPAMT , C.REGID,C.JOBCDT", conn);
