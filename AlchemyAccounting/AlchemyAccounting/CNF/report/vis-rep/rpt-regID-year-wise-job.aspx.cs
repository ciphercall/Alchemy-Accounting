...
namespace AlchemyAccounting.CNF.report.vis_rep
{
    public partial class rpt_regID_year_wise_job : System.Web.UI.Page
    {
        ...
        public void ShowGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand(@"SELECT CNF_JOB.JOBNO, GL_ACCHART.ACCOUNTNM, CONVERT(NVARCHAR(20), CNF_JOB.JOBCDT,103) AS JOBCDT, 
            CNF_JOB.GOODS,  BENO , CONVERT(NVARCHAR,BEDT,103) BEDT, 
            CNF_JOB.PKGS, CNF_JOB.PERMITNO, CNF_JOB.DOCINVNO, ASL_COMPANY.COMPNM, CNF_JOB.JOBTP,(CASE  
            WHEN CNF_JOB.DOCRCVDT ='1900-01-01' THEN NULL ELSE CONVERT(NVARCHAR(20), CNF_JOB.DOCRCVDT, 
            103) END) AS DOCRCVDT,CNF_JOB.CNFV_USD, (CASE WHEN CNF_JOB.PERMITDT ='1900-01-01' THEN NULL ELSE CONVERT(NVARCHAR(20),  
            CNF_JOB.PERMITDT, 103) END) AS PERMITDT, ASL_COMPANY.BRANCHID, CNF_JOB.LCNO  FROM  CNF_JOB INNER JOIN  
            ASL_COMPANY ON CNF_JOB.COMPID = ASL_COMPANY.COMPID INNER JOIN GL_ACCHART ON CNF_JOB.PARTYID = GL_ACCHART.ACCOUNTCD 
