...
namespace AlchemyAccounting.CNF.report.vis_rep
{
    public partial class RptExpenseRegCat : System.Web.UI.Page
    {
        ...
        public void ShowGrid()
        {
            ...
            Global.lblAdd("SELECT LCNO FROM CNF_JOB WHERE JOBNO=" + jobno + " AND JOBTP='" + jobty + "' AND JOBYY=" + jobyy + "", lblLcNo);
            Global.lblAdd("SELECT PKGS FROM CNF_JOB WHERE JOBNO=" + jobno + " AND JOBTP='" + jobty + "' AND JOBYY=" + jobyy + "", lblQty);
            ...
            SqlCommand cmd = new SqlCommand(" SELECT CNF_JOBEXP.EXPCD, COUNT(*) AS Qty, SUM(CNF_JOBEXP.EXPAMT) AS EXPAMT, GL_ACCHART.ACCOUNTNM + '----- Invoice No: '+CONVERT(NVARCHAR(20),CNF_JOBEXP.TRANSNO,103) + '----- Date : ' +CONVERT(NVARCHAR(20),CNF_JOBEXP.TRANSDT,103) AS ACCOUNTNM,  " +
                " CNF_EXPENSE.EXPNM, CNF_JOBEXP.EXPID, CNF_JOBEXP.TRANSNO, CNF_JOBEXP.TRANSDT FROM CNF_JOBEXP INNER JOIN CNF_EXPENSE ON CNF_JOBEXP.EXPID = CNF_EXPENSE.EXPID INNER JOIN " +
                " GL_ACCHART ON CNF_JOBEXP.EXPCD = GL_ACCHART.ACCOUNTCD " +
                " WHERE CNF_JOBEXP.JOBNO='" + jobno + "' AND CNF_JOBEXP.JOBTP='" + jobty + "' AND CNF_JOBEXP.JOBYY ='" + jobyy + "' " +
                " GROUP BY CNF_JOBEXP.EXPCD, GL_ACCHART.ACCOUNTNM, CNF_EXPENSE.EXPNM, CNF_JOBEXP.EXPID, CNF_JOBEXP.TRANSNO, CNF_JOBEXP.TRANSDT ORDER BY TRANSDT, TRANSNO, ACCOUNTNM", conn);
