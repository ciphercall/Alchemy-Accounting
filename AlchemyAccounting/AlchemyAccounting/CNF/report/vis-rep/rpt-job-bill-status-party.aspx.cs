...
{
    public partial class rpt_job_bill_status_party : System.Web.UI.Page
    {
        decimal expAmount = 0;
        ...
        public void showGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand(" SELECT A.JOBNO, CONVERT(NVARCHAR(20),A.BILLDT,103) AS BILLDT, A.BILLNO, A.EXPAMT, A.BILLAMT, A.JOBYY, A.JOBTP, SUM(ISNULL(CNF_JOBRCV.AMOUNT,0)) AS RCVAMT, (A.BILLAMT - SUM(ISNULL(CNF_JOBRCV.AMOUNT,0))) AS DUEAMT, A.ACCOUNTNM " +
                   ...
                   " WHERE (CNF_JOBBILL.JOBYY = " + JOBYY + ") AND (CNF_JOBBILL.JOBTP = '" + JOBTP + "') AND (CNF_JOBBILL.COMPID='" + Companyid + "') AND (GL_ACCHART.ACCOUNTCD='" + partyid + "') GROUP BY CNF_JOBBILL.JOBNO, CNF_JOBBILL.BILLDT, CNF_JOBBILL.BILLNO, CNF_JOB.COMM_AMT, CNF_JOBBILL.JOBYY, CNF_JOBBILL.JOBTP, GL_ACCHART.ACCOUNTNM) AS A LEFT OUTER JOIN " +
