...
{
    public partial class rpt_date_wise_job : System.Web.UI.Page
    {
        ...
        public void ShowGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand(@"SELECT CNF_JOB.JOBNO, GL_ACCHART.ACCOUNTNM, CNF_JOB.REGID, CNF_JOB.GOODS, CNF_JOB.PKGS, CNF_JOB.BENO PERMITNO, 
CNF_JOB.DOCINVNO,  CNF_JOB.LCNO COMPNM, CNF_JOB.JOBYY, CNF_JOB.JOBTP,CNF_JOB.CNFV_USD,
(CASE WHEN CNF_JOB.DOCRCVDT ='1900-01-01' THEN NULL ELSE CONVERT(NVARCHAR(20), CNF_JOB.DOCRCVDT, 103) END) AS DOCRCVDT, 
(CASE WHEN CNF_JOB.BEDT ='1900-01-01' THEN NULL ELSE CONVERT(NVARCHAR(20), CNF_JOB.BEDT, 103) END) AS PERMITDT,
ASL_COMPANY.BRANCHID  FROM  CNF_JOB INNER JOIN  ASL_COMPANY ON CNF_JOB.COMPID = ASL_COMPANY.COMPID  INNER JOIN " +
                     " GL_ACCHART ON CNF_JOB.PARTYID = GL_ACCHART.ACCOUNTCD " +
                     " WHERE CNF_JOB.JOBCDT BETWEEN @FROMDATE AND @TODATE ", conn);
