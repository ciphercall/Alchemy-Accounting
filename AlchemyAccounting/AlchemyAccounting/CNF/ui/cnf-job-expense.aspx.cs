...
namespace AlchemyAccounting.CNF.ui
{
    public partial class cnf_job_expense : System.Web.UI.Page
    {
        ...
        public static string[] GetCompletionListJob_No_Year_Type(string prefixText, int count, string contextKey)
        {
            ...
            if (uTp == "ADMIN")
            {
                cmd.CommandText = ("SELECT (CONVERT(NVARCHAR(20),JOBNO,103) + '|' + CONVERT(NVARCHAR(20),JOBYY,103) + '|' + JOBTP) AS JOBPAR FROM CNF_JOB WHERE (CONVERT(NVARCHAR(20),JOBNO,103) + '|' + CONVERT(NVARCHAR(20),JOBYY,103) + '|' + JOBTP) LIKE '" + prefixText + "%'  ORDER BY JOBYY DESC, JOBNO");
            }
            else
            {
                cmd.CommandText = ("SELECT (CONVERT(NVARCHAR(20),JOBNO,103) + '|' + CONVERT(NVARCHAR(20),JOBYY,103) + '|' + JOBTP) AS JOBPAR FROM CNF_JOB WHERE (CONVERT(NVARCHAR(20),JOBNO,103) + '|' + CONVERT(NVARCHAR(20),JOBYY,103) + '|' + JOBTP) LIKE '" + prefixText + "%' AND COMPID ='" + brCD + "' ORDER BY JOBYY DESC, JOBNO");
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (ds1.Tables[0].Rows.Count > 1)
                    {
                        ...
                        SqlCommand cmdselectdata = new SqlCommand("Select * FROM CNF_JOBEXP WHERE TRANSMY =@TRANSMY AND TRANSNO =@TRANSNO AND SLNO =@SLNO", conn);

                        cmdselectdata.Parameters.Clear();
                        cmdselectdata.Parameters.Add("@TRANSMY", SqlDbType.NVarChar).Value = iob.ExMY;
                        cmdselectdata.Parameters.Add("@TRANSNO", SqlDbType.BigInt).Value = iob.InvoiceNO;
                        cmdselectdata.Parameters.Add("@SLNO", SqlDbType.BigInt).Value = iob.Sl;
                        ...
                        string alldata = TRANSDT + ", " + TRANSMY + ", " + TRANSNO
                            + ", " + COMPID + ", " + JOBYY + ", " + JOBTP + ", " + JOBNO + ", " + EXPCD + ", " + SLNO + ", " + EXPID + ", " + EXPAMT
                            + ", " + REMARKS + ", " + USERPC + ", " + USERID + ", " + UPDATEUSERID + ", " + INTIME + ", " + ACTDTI + ", " + UPDATETIME + ", " + IPADDRESS;


                        SqlCommand cmdinsert = new SqlCommand("insert into ASL_DLT values('CNF_JOBEXP',@DESCRP,@USERPC,@USERID,@INTIME,@IPADD)", conn);
                    ...
                    else
                    {
                        ...
                        cmdinsert.ExecuteNonQuery();    //insert delete_cnf_job_expense


                        dob.delete_cnf_job_expense(iob); // delete delete_cnf_job_expense


                        SqlCommand cmdselectdatamst = new SqlCommand("Select * FROM CNF_JOBEXPMST WHERE TRANSMY =@TRANSMY AND TRANSNO =@TRANSNO", conn);

                        cmdselectdatamst.Parameters.Clear();
                        cmdselectdatamst.Parameters.Add("@TRANSMY", SqlDbType.NVarChar).Value = iob.ExMY;
                        cmdselectdatamst.Parameters.Add("@TRANSNO", SqlDbType.BigInt).Value = iob.InvoiceNO;

                        SqlDataReader drmst = cmdselectdata.ExecuteReader();
                        while (drmst.Read())
                        {
                            TRANSDT = drmst["TRANSDT"].ToString();
                            TRANSMY = drmst["TRANSMY"].ToString();
                            TRANSNO = drmst["TRANSNO"].ToString();
                            COMPID = drmst["COMPID"].ToString();
                            JOBYY = drmst["JOBYY"].ToString();
                            JOBTP = drmst["JOBTP"].ToString();
                            JOBNO = drmst["JOBNO"].ToString();
                            EXPCD = drmst["EXPCD"].ToString();
                            REMARKS = drmst["REMARKS"].ToString();
                            USERPC = drmst["USERPC"].ToString();
                            USERID = drmst["USERID"].ToString();
                            UPDATEUSERID = drmst["UPDATEUSERID"].ToString();
                            ACTDTI = drmst["ACTDTI"].ToString();
                            INTIME = drmst["INTIME"].ToString();
                            UPDATETIME = drmst["UPDATETIME"].ToString();
                            IPADDRESS = drmst["IPADDRESS"].ToString();

                        }
                        drmst.Close();

                        string alldataMST = TRANSDT + ", " + TRANSMY + ", " + TRANSNO
                            + ", " + COMPID + ", " + JOBYY + ", " + JOBTP + ", " + JOBNO + ", " + EXPCD
                            + ", " + REMARKS + ", " + USERPC + ", " + USERID + ", " + UPDATEUSERID
                            + ", " + INTIME + ", " + ACTDTI + ", " + UPDATETIME + ", " + IPADDRESS;


                        SqlCommand cmdinsertmst = new SqlCommand("insert into ASL_DLT values('CNF_JOBEXPMST',@DESCRP,@USERPC,@USERID,@INTIME,@IPADD)", conn);
                        cmdinsertmst.Parameters.AddWithValue("@DESCRP", alldataMST);
                        ...
                        cmdinsertmst.ExecuteNonQuery();    //insert delete_cnf_job_expense
        ...
        protected void ddlInvoice_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Global.txtAdd("SELECT CONVERT(NVARCHAR,TRANSDT,103) AS TRANSDT FROM CNF_JOBEXPMST WHERE TRANSMY ='" + lblMY.Text + "' AND TRANSNO =" + ddlInvoice.Text + "", txtExDT);
                GridShow();
        ...
        {
            var voucherNo = "";
            var monthyy = lblMY.Text;
            voucherNo = btnEdit.Text == "Edit" ? txtNo.Text : ddlInvoice.Text;

            string countMasterData = Global.StringData("SELECT COUNT(*) CNT FROM CNF_JOBEXPMST WHERE TRANSMY ='" + monthyy + "' AND TRANSNO =" + voucherNo + "");
            if (countMasterData == "1")
            {
                string countChildData = Global.StringData("SELECT COUNT(*) CNT FROM CNF_JOBEXP WHERE TRANSMY ='" + monthyy + "' AND TRANSNO =" + voucherNo + "");
                if (countChildData == "0")
                {
                    try
                    {
                        ...
                        cmdselectdatamst.Parameters.Clear();
                        cmdselectdatamst.Parameters.Add("@TRANSMY", SqlDbType.NVarChar).Value = monthyy;
                        cmdselectdatamst.Parameters.Add("@TRANSNO", SqlDbType.BigInt).Value = voucherNo;
                        conn.Open();
                    ...
                    var s = Global.ExecuteCommand("DELETE FROM CNF_JOBEXPMST WHERE TRANSMY ='" + monthyy + "' AND TRANSNO =" + voucherNo + "");
                    if (s == "")
                    {
                        Refresh();
                        lblError.Text = "Deleted Succesfully.";
                        lblError.Visible = true;
                        ...
                        else
                        {
                            ...
                            if (uTp == "ADMIN")
                            {
                                Global.dropDownAddWithSelect(ddlInvoice,
                                    "SELECT TRANSNO FROM CNF_JOBEXPMST WHERE TRANSMY ='" + lblMY.Text + "'");
                            }
                            else
                            {
                                Global.dropDownAddWithSelect(ddlInvoice,
                    ...
                    {
                        lblError.Text = "Child data available in this voucher.";
                        lblError.Visible = true;
                    }
                }
                else
            ...
            {
                lblError.Text = "Select proper voucher no.";
                lblError.Visible = true;
        ...
        protected void btnPrint_ClickMini(object sender, EventArgs e)
        {
            RefreshSession();
            ...
            else if (ddlInvoice.Text == "Select" && txtNo.Text == "")
            {
                lblError.Visible = true;
                lblError.Text = "Select expense.";
                ddlInvoice.Focus();
            ...
            else
            {
                ...
                iob.ExMY = lblMY.Text;
                Session["transmy"] = iob.ExMY;
                iob.InvoiceNO = Convert.ToInt64(btnEdit.Text == "Edit" ? txtNo.Text : ddlInvoice.Text);
                Session["InvoiceNo"] = iob.InvoiceNO;
                ...
                Page.ClientScript.RegisterStartupScript(
                this.GetType(), "OpenWindow", "window.open('../report/vis-rep/RptExpense-VR-mini.aspx','_newtab');", true);
            }
        }
