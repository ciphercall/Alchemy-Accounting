    ...
    public partial class JobStatus : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    if (Session["UserTp"].ToString() == "USER" || Session["BrCD"].ToString() == "C02")
                    {
        ...
        protected void gvDetails_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            GridShow();
            TextBox txtFwdDateEdit = (TextBox)gvDetails.Rows[e.NewEditIndex].FindControl("txtFwdDateEdit");
            txtFwdDateEdit.Focus();
        }
        ...
        protected void btnprocess_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                conn = new SqlConnection(Global.connection);
                conn.Open();
                cmdd = new SqlCommand("Delete from CNF_JOBBILL where COMPID ='" + txtCompID.Text + "' AND JOBYY =" + txtJobYR.Text + " AND JOBTP ='" + ddlJobTP.Text + "' AND JOBNO =" + jobno + "  AND BILLAMT<=0", conn);
                ...
                foreach (GridViewRow grid in GridView1.Rows)
                {
                    try
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            if (jsm.ExpID.Substring(0, 2) == "I1")
                                jsda.Job_Bill_ProcessForI1BillAmount(jsm);
                            else
        ...
        public void ShowGrid_Bill_Process(Int64 jNo)
        {
            ...
            conn.Open();
            SqlCommand cmd = new SqlCommand(" SELECT CNF_JOBEXP.COMPID, CNF_JOBSTATUS.TRANSDT AS BILLDATE, CNF_JOBSTATUS.TRANSNO AS BILLNO, CNF_JOB.PARTYID, CNF_JOBEXP.EXPID, SUM(CNF_JOBEXP.EXPAMT) AS AMOUNT, CNF_JOBEXP.USERID " +
                       ...
                       " GROUP BY CNF_JOBEXP.COMPID, CNF_JOBEXP.EXPID, CNF_JOBSTATUS.TRANSDT, CNF_JOBSTATUS.TRANSNO, CNF_JOB.PARTYID, CNF_JOBEXP.USERID", conn);
