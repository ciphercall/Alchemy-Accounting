...
{
    public partial class rptReceiptPaymentStateAccessSelect : System.Web.UI.Page
    {
        ...
        public void showGrid_OP()
        {
            ...
            string From = Session["From"].ToString();
            string To = Session["To"].ToString();
            string accesscode = Session["AccessCd"].ToString();
            ...
            if (uTp == "ADMIN")
            {
                cmd = new SqlCommand("SELECT A.SL, A.HEADER, A.ACCD1, A.ACCD2, A.DRAMT, A.CRAMT, GL_ACCHART.ACCOUNTNM, GL_ACCHART_1.ACCOUNTNM AS ACCOUNTNM2 " +
                                                        ...
                                                        " FROM GL_MASTER " +
                                                        " WHERE      (TRANSDT < '" + FrDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') " +
                                                        " GROUP BY DEBITCD " +
                                                        " UNION " +
                                                        ...
                                                        " WHERE     (TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') AND (TRANSTP = 'MREC') " +
                                                        " GROUP BY CREDITCD " +
                                                        " UNION " +
                                                        ...
                                                        " WHERE     (TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (SUBSTRING(CREDITCD, 1, 5) = '10201') AND (TRANSTP = 'MPAY') " +
                                                        " GROUP BY DEBITCD " +
                                                        " UNION " +
                                                        ...
                                                        " WHERE     (TRANSDT <= '" + ToDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') " +
                                                        " GROUP BY DEBITCD) AS A INNER JOIN " +
                                                " GL_ACCHART ON A.ACCD2 = GL_ACCHART.ACCOUNTCD AND '"+accesscode+"'= (CASE WHEN ISNULL(GL_ACCHART.BRANCHCD,'')='' THEN 'C01' ELSE GL_ACCHART.BRANCHCD END)  INNER JOIN " +
                                                " GL_ACCHART AS GL_ACCHART_1 ON A.ACCD1 = GL_ACCHART_1.ACCOUNTCD " +
                                                " WHERE A.HEADER ='OPENING BALANCE' " +
                                                "ORDER BY A.SL,GL_ACCHART_1.ACCOUNTNM", conn);
            }
            else
            {
                cmd = new SqlCommand("SELECT A.SL, A.HEADER, A.ACCD1, A.ACCD2, A.DRAMT, A.CRAMT, GL_ACCHART.ACCOUNTNM, GL_ACCHART_1.ACCOUNTNM AS ACCOUNTNM2 " +
                                                        " FROM (SELECT 'OPENING BALANCE' AS HEADER, 1 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, SUM(isnull(DEBITAMT,0)) - SUM(isnull(CREDITAMT,0)) AS DRAMT, 0 AS CRAMT " +
                                                        " FROM GL_MASTER " +
                                                        " WHERE      (TRANSDT < '" + FrDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') AND (SUBSTRING(COSTPID, 1, 3) ='" + brCD + "') " +
                                                        " GROUP BY DEBITCD " +
                                                        " UNION " +
                                                        " SELECT 'RECEIVED DURING THE PERIOD' AS HEADER, 2 AS SL, SUBSTRING(CREDITCD, 1, 7) + '00000' AS ACCD1, CREDITCD AS ACCD2, SUM(isnull(DEBITAMT,0)) AS DRAMT, 0 AS CRAMT " +
                                                        " FROM         GL_MASTER AS GL_MASTER_3 " +
                                                        " WHERE     (TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') AND (TRANSTP = 'MREC') AND (SUBSTRING(COSTPID, 1, 3) ='" + brCD + "') " +
                                                        " GROUP BY CREDITCD " +
                                                        " UNION " +
                                                        " SELECT     'PAYMENT DURING THE PERIOD' AS 'HEADER', 3 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 0 AS DRAMT, SUM(isnull(DEBITAMT,0)) AS CRAMT " +
                                                        " FROM         GL_MASTER AS GL_MASTER_2 " +
                                                        " WHERE     (TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (SUBSTRING(CREDITCD, 1, 5) = '10201') AND (TRANSTP = 'MPAY') AND (SUBSTRING(COSTPID, 1, 3) ='" + brCD + "') " +
                                                        " GROUP BY DEBITCD " +
                                                        " UNION " +
                                                        " SELECT     'CLOSING BALANCE' AS HEADER, 4 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 0 AS DRAMT, SUM(isnull(DEBITAMT,0)) - SUM(CREDITAMT) AS CRAMT " +
                                                        " FROM         GL_MASTER AS GL_MASTER_1 " +
                                                        " WHERE     (TRANSDT <= '" + ToDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') AND (SUBSTRING(COSTPID, 1, 3) ='" + brCD + "') " +
                                                        " GROUP BY DEBITCD) AS A INNER JOIN " +
                                                " GL_ACCHART ON A.ACCD2 = GL_ACCHART.ACCOUNTCD  AND '" + accesscode + "'= (CASE WHEN ISNULL(GL_ACCHART.BRANCHCD,'')='' THEN 'C01' ELSE GL_ACCHART.BRANCHCD END) INNER JOIN " +
                                                " GL_ACCHART AS GL_ACCHART_1 ON A.ACCD1 = GL_ACCHART_1.ACCOUNTCD " +
                                                " WHERE A.HEADER ='OPENING BALANCE' " +
        ...
        public void showGrid_MREC()
        {
            ...
            if (uTp == "ADMIN")
            {
                cmd = new SqlCommand(@"SELECT A.SL, A.HEADER, A.ACCD1, A.ACCD2, A.DRAMT, A.CRAMT, GL_ACCHART.ACCOUNTNM, GL_ACCHART_1.ACCOUNTNM AS ACCOUNTNM2 " +
                                                      " FROM (SELECT 'OPENING BALANCE' AS HEADER, 1 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, SUM(isnull(DEBITAMT,0)) - SUM(isnull(CREDITAMT,0)) AS DRAMT, 0 AS CRAMT " +
                                                      " FROM GL_MASTER  INNER JOIN GL_ACCHART B ON GL_MASTER.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='" + accesscode + "' " +
                                                     " WHERE      (TRANSDT < '" + FrDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') " +
                                                      " GROUP BY DEBITCD " +
                                                      " UNION " +
                                                      " SELECT 'RECEIVED DURING THE PERIOD' AS HEADER, 2 AS SL, SUBSTRING(CREDITCD, 1, 7) + '00000' AS ACCD1, CREDITCD AS ACCD2, SUM(isnull(DEBITAMT,0)) AS DRAMT, 0 AS CRAMT " +
                                                      " FROM         GL_MASTER AS GL_MASTER_3  INNER JOIN GL_ACCHART B ON GL_MASTER_3.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='" + accesscode + "' " +
                                                      " WHERE     (TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') AND (TRANSTP = 'MREC') " +
                                                      " GROUP BY CREDITCD " +
                                                      " UNION " +
                                                      " SELECT     'PAYMENT DURING THE PERIOD' AS 'HEADER', 3 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 0 AS DRAMT, SUM(isnull(DEBITAMT,0)) AS CRAMT " +
                                                      " FROM         GL_MASTER AS GL_MASTER_2  INNER JOIN GL_ACCHART B ON GL_MASTER_2.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='" + accesscode + "' " +
                                                      " WHERE     (TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (SUBSTRING(CREDITCD, 1, 5) = '10201') AND (TRANSTP = 'MPAY') " +
                                                      " GROUP BY DEBITCD " +
                                                      " UNION " +
                                                      " SELECT     'CLOSING BALANCE' AS HEADER, 4 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 0 AS DRAMT, SUM(isnull(DEBITAMT,0)) - SUM(CREDITAMT) AS CRAMT " +
                                                      " FROM         GL_MASTER AS GL_MASTER_1  INNER JOIN GL_ACCHART B ON GL_MASTER_1.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='" + accesscode + "' " +
                                                      " WHERE     (TRANSDT <= '" + ToDT + "') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') " +
                                                      " GROUP BY DEBITCD) AS A INNER JOIN " +
        ...
        public void showGrid_MPAY()
        {
            ...
            if (uTp == "ADMIN")
            {
                cmd = new SqlCommand(@"SELECT A.SL, A.HEADER, A.ACCD1, A.ACCD2, A.DRAMT, A.CRAMT, GL_ACCHART.ACCOUNTNM, GL_ACCHART_1.ACCOUNTNM AS ACCOUNTNM2  FROM 
                (
                SELECT 'OPENING BALANCE' AS HEADER, 1 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 
                SUM(isnull(DEBITAMT,0)) - SUM(isnull(CREDITAMT,0)) AS DRAMT, 0 AS CRAMT  
                FROM GL_MASTER  INNER JOIN GL_ACCHART B ON GL_MASTER.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='{accesscode}'
                WHERE      (TRANSDT < '{FrDT}') AND (SUBSTRING(DEBITCD, 1, 5) = '10201')  GROUP BY DEBITCD  
                UNION  
                SELECT 'RECEIVED DURING THE PERIOD' AS HEADER, 2 AS SL, SUBSTRING(CREDITCD, 1, 7) + '00000' AS ACCD1, CREDITCD AS ACCD2, 
                SUM(isnull(DEBITAMT,0)) AS DRAMT, 0 AS CRAMT  
                FROM         GL_MASTER AS GL_MASTER_3  INNER JOIN GL_ACCHART B ON GL_MASTER_3.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='{accesscode}'
                WHERE   (TRANSDT BETWEEN '{FrDT}' AND '{ToDT}') AND (SUBSTRING(DEBITCD, 1, 5) = '10201') AND (TRANSTP = 'MREC')  
                GROUP BY CREDITCD  
                UNION  
                SELECT 'PAYMENT DURING THE PERIOD' AS 'HEADER', 3 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 0 AS DRAMT, 
                SUM(isnull(DEBITAMT,0)) AS CRAMT  
                FROM   GL_MASTER AS GL_MASTER_2  INNER JOIN GL_ACCHART B ON GL_MASTER_2.CREDITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='{accesscode}'
                WHERE  (TRANSDT BETWEEN '{FrDT}' AND '{ToDT}') AND (SUBSTRING(CREDITCD, 1, 5) = '10201') AND (TRANSTP = 'MPAY')  
                GROUP BY DEBITCD  
                UNION  
                SELECT 'CLOSING BALANCE' AS HEADER, 4 AS SL, SUBSTRING(DEBITCD, 1, 7) + '00000' AS ACCD1, DEBITCD AS ACCD2, 0 AS DRAMT, 
                SUM(isnull(DEBITAMT,0)) - SUM(CREDITAMT) AS CRAMT  
                FROM         GL_MASTER AS GL_MASTER_1  INNER JOIN GL_ACCHART B ON GL_MASTER_1.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD,'C01')='{accesscode}'
                WHERE     (TRANSDT <= '{ToDT}') AND (SUBSTRING(DEBITCD, 1, 5) = '10201')  
                GROUP BY DEBITCD) AS A INNER JOIN  GL_ACCHART ON A.ACCD2 = GL_ACCHART.ACCOUNTCD 
                INNER JOIN  GL_ACCHART AS GL_ACCHART_1 ON A.ACCD1 = GL_ACCHART_1.ACCOUNTCD  
                WHERE A.HEADER ='PAYMENT DURING THE PERIOD' ORDER BY A.SL,GL_ACCHART_1.ACCOUNTNM", conn);
            }
            else
            {
                cmd = new SqlCommand(@"SELECT A.SL, A.HEADER, A.ACCD1, A.ACCD2, A.DRAMT, A.CRAMT, GL_ACCHART.ACCOUNTNM, GL_ACCHART_1.ACCOUNTNM AS ACCOUNTNM2  FROM 
                ...
                SUM(isnull(DEBITAMT, 0)) - SUM(isnull(CREDITAMT, 0)) AS DRAMT, 0 AS CRAMT
                FROM GL_MASTER  INNER JOIN GL_ACCHART B ON GL_MASTER.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD, 'C01') = '{accesscode}'
                WHERE(TRANSDT < '{FrDT}') AND(SUBSTRING(DEBITCD, 1, 5) = '10201') AND (SUBSTRING(COSTPID, 1, 3) ='{brCD}')   GROUP BY DEBITCD
                UNION
                SELECT 'RECEIVED DURING THE PERIOD' AS HEADER, 2 AS SL, SUBSTRING(CREDITCD, 1, 7) + '00000' AS ACCD1, CREDITCD AS ACCD2,
                SUM(isnull(DEBITAMT, 0)) AS DRAMT, 0 AS CRAMT
                FROM         GL_MASTER AS GL_MASTER_3  INNER JOIN GL_ACCHART B ON GL_MASTER_3.DEBITCD = B.ACCOUNTCD AND ISNULL(B.BRANCHCD, 'C01') = '{accesscode}'
        ...
        protected void GridView4_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                if (dblGrandTotalDRAMTComma_OP == "")
                    dblGrandTotalDRAMTComma_OP = "0.00";
                decimal G_Total_OP = Convert.ToDecimal(dblGrandTotalDRAMTComma_OP);
                decimal G_Total_MREC = Convert.ToDecimal(dblGrandTotalDRAMTComma_MREC);
                decimal G_Total_MPAY = Convert.ToDecimal(dblGrandTotalDRAMTComma_MPAY);
                decimal G_Total_CL = Convert.ToDecimal(dblGrandTotalDRAMTComma_CL);
                ...
                decimal G_Total_CR_OP = Convert.ToDecimal(dblGrandTotalCRAMTComma_OP);
                decimal G_Total_CR_MREC = Convert.ToDecimal(dblGrandTotalCRAMTComma_MREC);
                decimal G_Total_CR_MPAY = Convert.ToDecimal(dblGrandTotalCRAMTComma_MPAY);
                decimal G_Total_CR_CL = Convert.ToDecimal(dblGrandTotalCRAMTComma_CL);
