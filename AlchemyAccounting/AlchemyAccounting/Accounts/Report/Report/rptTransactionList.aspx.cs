...
namespace AlchemyAccounting.Accounts.Report.Report
{
    public partial class rptTransactionList : System.Web.UI.Page
    {
        ...
        string dblGrandTotalAmountComma_MREC = "0";
        string dblGrandTotalAmountComma_MPAY = "0";
        string dblGrandTotalAmountComma_JOUR = "0";
        string dblGrandTotalAmountComma_CONT = "0";
        ...
        public void showGrid_Credit()
        {
            ...
            if (uTp == "ADMIN")
            {
                cmd = new SqlCommand(" SELECT GL_MASTER.TRANSTP,(CASE WHEN GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS') THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1) " +
                                              ...
                                              " 'APV ' END) " +
                                              " WHEN GL_MASTER.TABLEID = 'STK_TRANS' then 'AJV ' WHEN GL_MASTER.TABLEID = 'LC_EXPENSE' THEN 'APV ' ELSE 'APV ' END)+'- '+  " +
                                  ...
                                  " WHERE (GL_MASTER.TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS')) AND GL_MASTER.TRANSDRCR='DEBIT' and GL_MASTER.TRANSTP = 'MREC'" +
        ...
        public void showGrid_Debit()
        {
            ...
            {
                cmd = new SqlCommand(@" SELECT GL_MASTER.TRANSTP,(CASE WHEN GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS') THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1)  + 
                                    (CASE WHEN GL_MASTER.TRANSTP = 'MREC' THEN 'RV ' WHEN GL_MASTER.TRANSTP = 'MPAY' THEN 'PV ' WHEN GL_MASTER.TRANSTP = 'JOUR' 
                                    THEN 'JV ' WHEN GL_MASTER.TRANSTP = 'CONT' THEN 'CV ' ELSE  'APV ' END)  WHEN GL_MASTER.TABLEID = 'STK_TRANS' then 'AJV ' 
                                    WHEN GL_MASTER.TABLEID = 'LC_EXPENSE' THEN 'APV ' ELSE 'APV ' END)+'- '+   + CONVERT(nvarchar(10), GL_MASTER.TRANSNO, 103)
                                    AS DOCNO, convert(nvarchar(20),GL_MASTER.TRANSDT,105) as TRANSDT, GL_MASTER.TRANSNO, GL_MASTER.DEBITCD, GL_MASTER.CREDITCD, 
                                    GL_MASTER.DEBITAMT,  GL_MASTER.CHEQUENO, GL_MASTER.CHEQUEDT, GL_MASTER.REMARKS, GL_MASTER.TABLEID, GL_ACCHART.ACCOUNTNM AS 
                                    DebitHead,  GL_ACCHART_1.ACCOUNTNM AS CreditHead, GL_MASTER.TRANSMODE, GL_MASTER.TRANSDRCR  FROM GL_MASTER INNER JOIN 
                                    GL_ACCHART ON GL_MASTER.DEBITCD = GL_ACCHART.ACCOUNTCD INNER JOIN  GL_ACCHART AS GL_ACCHART_1 ON GL_MASTER.CREDITCD = 
                                    GL_ACCHART_1.ACCOUNTCD "+
                                  " WHERE (GL_MASTER.TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (GL_MASTER.TABLEID IN ('GL_STRANS','LC_EXPENSE','GL_MTRANS')) AND GL_MASTER.TRANSDRCR='DEBIT' and GL_MASTER.TRANSTP = 'MPAY'" +
                                  " ORDER BY TRANSDT, TRANSTP DESC,TRANSNO ASC", conn);
            }
            else
            {
        ...
        public void showGrid_Journal()
        {
            ...
            if (uTp == "ADMIN")
            {
                cmd = new SqlCommand(" SELECT GL_MASTER.TRANSTP,(CASE WHEN GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS') THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1) " +
                                  ...
                                  " GL_ACCHART ON GL_MASTER.DEBITCD = GL_ACCHART.ACCOUNTCD INNER JOIN " +
                                  " GL_ACCHART AS GL_ACCHART_1 ON GL_MASTER.CREDITCD = GL_ACCHART_1.ACCOUNTCD " +
                                  " WHERE (GL_MASTER.TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (GL_MASTER.TABLEID IN ('GL_STRANS','STK_TRANS','GL_MTRANS')) AND GL_MASTER.TRANSDRCR='DEBIT' and GL_MASTER.TRANSTP = 'JOUR'" +
                                  " ORDER BY TRANSDT, TRANSTP DESC,TRANSNO ASC", conn);
            }
            else
            {
                cmd = new SqlCommand(" SELECT GL_MASTER.TRANSTP,(CASE WHEN GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS') THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1) " +
                                 ...
                                 " GL_ACCHART ON GL_MASTER.DEBITCD = GL_ACCHART.ACCOUNTCD INNER JOIN " +
                                 " GL_ACCHART AS GL_ACCHART_1 ON GL_MASTER.CREDITCD = GL_ACCHART_1.ACCOUNTCD " +
                                 " WHERE (GL_MASTER.TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (GL_MASTER.TABLEID IN ('GL_STRANS','STK_TRANS')) AND GL_MASTER.TRANSDRCR='DEBIT' and GL_MASTER.TRANSTP = 'JOUR' AND (SUBSTRING(GL_MASTER.COSTPID, 1,3)='" + brCD + "')" +
                                 " ORDER BY TRANSDT, TRANSTP DESC,TRANSNO ASC", conn);
            }
        ...
        public void showGrid_Contra()
        {
            ...
            if (uTp == "ADMIN")
            {
                cmd = new SqlCommand(" SELECT GL_MASTER.TRANSTP,(CASE WHEN GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS') THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1) " +
                                   ...
                                   " GL_ACCHART ON GL_MASTER.DEBITCD = GL_ACCHART.ACCOUNTCD INNER JOIN " +
                                   " GL_ACCHART AS GL_ACCHART_1 ON GL_MASTER.CREDITCD = GL_ACCHART_1.ACCOUNTCD " +
                                   " WHERE (GL_MASTER.TRANSDT BETWEEN '" + FrDT + "' AND '" + ToDT + "') AND (GL_MASTER.TABLEID IN ('GL_STRANS','GL_MTRANS')) AND GL_MASTER.TRANSDRCR='DEBIT' and GL_MASTER.TRANSTP = 'CONT'" +
                                   " ORDER BY TRANSDT, TRANSTP DESC,TRANSNO ASC", conn);
            }
            else
            {
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                e.Row.Cells[2].Text = "Total Number of Voucher :   " + RowNo;
                e.Row.Cells[2].HorizontalAlign = HorizontalAlign.Center;
                e.Row.Cells[3].Text = "Total :";
                e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Cells[4].Text = dblGrandTotalAmountComma_MREC;
        ...
        protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                e.Row.Cells[3].Text = "Total :";
                e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Cells[4].Text = dblGrandTotalAmountComma_MPAY;
                e.Row.Cells[4].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Font.Bold = true;
            }
        ...
        protected void GridView3_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                e.Row.Cells[3].Text = "Total :";
                e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Cells[4].Text = dblGrandTotalAmountComma_JOUR;
                e.Row.Cells[4].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Font.Bold = true;
            }
        ...
        protected void GridView4_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                e.Row.Cells[3].Text = "Total :";
                e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Cells[4].Text = dblGrandTotalAmountComma_CONT;
                e.Row.Cells[4].HorizontalAlign = HorizontalAlign.Right;
                e.Row.Font.Bold = true;
            }
