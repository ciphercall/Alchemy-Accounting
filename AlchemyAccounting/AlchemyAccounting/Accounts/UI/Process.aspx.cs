...
namespace AlchemyAccounting.Accounts.UI
{
    public partial class Process : System.Web.UI.Page
    {
        ...
        protected void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            else if (DateTime.Parse(txtDate.Text, dateformat, DateTimeStyles.AssumeLocal) <
                    DateTime.Parse("01/12/2016", dateformat, DateTimeStyles.AssumeLocal))
            {
                Response.Write("<script>alert('Please select date from Frist DEC 2016.');</script>");
            }
            else
            {
                if (txtDate.Text == "")
                ...
                else
                {
                    ...
                    foreach (GridViewRow grid in gridMultiple.Rows)
                    {
                        try
                        {
                            if (grid.Cells[0].Text == "MREC")
                            {
                                Global.lblAdd(@"Select max(SERIALNO) FROM  GL_MASTER where TRANSDT = '" + trans_DT + "' and TRANSTP = 'MREC' AND TABLEID ='GL_MTRANS' ", lblSerial_Mrec);
                                if (lblSerial_Mrec.Text == "")
                                {
                                    serialNo = "25000";
                                    iob.SerialNo_MREC = serialNo;
                            ...
                            {
                                Global.lblAdd(@"Select max(SERIALNO) FROM  GL_MASTER where TRANSDT = '" + trans_DT + "' and TRANSTP = 'MPAY' AND TABLEID ='GL_MTRANS'", lblSerial_Mpay);
                                if (lblSerial_Mpay.Text == "")
                                {
                                    serialNo = "26000";
                                    iob.SerialNo_MREC = serialNo;
                            ...
                            {
                                Global.lblAdd(@"Select max(SERIALNO) FROM  GL_MASTER where TRANSDT = '" + trans_DT + "' and TRANSTP = 'JOUR' AND TABLEID ='GL_MTRANS'", lblSerial_Jour);
                                if (lblSerial_Jour.Text == "")
                                {
                                    serialNo = "27000";
                                    iob.SerialNo_MREC = serialNo;
                            ...
                            {
                                Global.lblAdd(@"Select max(SERIALNO) FROM  GL_MASTER where TRANSDT = '" + trans_DT + "' and TRANSTP = 'CONT' AND TABLEID ='GL_MTRANS' ", lblSerial_Cont);
                                if (lblSerial_Cont.Text == "")
                                {
                                    serialNo = "28000";
                                    iob.SerialNo_MREC = serialNo;
